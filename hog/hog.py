...
def max_scoring_num_rolls(dice=six_sided, num_samples=1000):
    ...
    # BEGIN PROBLEM 9
    "*** YOUR CODE HERE ***"
    higherst_average_score = 0
    highest_dice_number = 0
    current_dice_number =1
    while current_dice_number <= 10:
        current_average_score = make_averaged(roll_dice, num_samples)(current_dice_number,dice)
        if current_average_score > higherst_average_score:
            higherst_average_score =current_average_score
            highest_dice_number = current_dice_number
        elif current_average_score == higherst_average_score:
            highest_dice_number = min(current_dice_number, highest_dice_number)
        current_dice_number +=1
...
def swap_strategy(score, opponent_score, margin=8, num_rolls=4):
    ...
    if is_swap(final_score, opponent_score):
        if final_score < opponent_score:
            return 0
        elif final_score == opponent_score and gain >= margin:
            return 0
        else:
            return num_rolls
    elif gain >= margin:
